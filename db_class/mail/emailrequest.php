<?php
	...
	//To
	$mail->addAddress('pinonlinebanking@gmail.com', 'Pin Support');
	//From
	$mail->addReplyTo($email, 'Pin Support');
	$mail->setFrom($email, "Pin Support");
	$mail->Subject = 'Email Me Request ['.$accountNumber.'] | Pin Online Banking';
	...
	$body .= "<td style='border:none;'><strong>Customer account holding: {$accountNumber} has requested email support. Proceed with the request immediately. Reply to this email or use following email address.</strong><br><br></td></td>";
	$body .= "<tr><td style='border:none;'>Customer Email:<strong> <div style='width: 300px; padding: 5px; border: 2px solid red; text-align: center;'><a href='mailto:{$email}'>{$email}</a></div></strong><br><br><br></td>";
	$body .= "</tr>";
